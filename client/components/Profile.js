...
import { fetchMovies } from "../store/allMoviesStore";
import { fetchUsers } from "../store/allUsersStore";
import { fetchFriends, updateSingleFriend } from "../store/allFriendsStore";
...
const Profile = () => {
  ...
  const stats = useMemo(() => {
    ...
    const lastMovie = watchedMovies.reduce((latest, movie) => {
      return !latest || new Date(movie.dateWatched) > new Date(latest.dateWatched) ? movie : latest;
    }, null);
    ...
    const highestRatedMovie = watchedMovies.reduce((highest, movie) => {
      return !highest || (movie.rating || 0) > (highest.rating || 0) ? movie : highest;
    }, null);
  ...
  return (
    <div className="profile-container">
      ...
      {user.image && <div className="user-image" style={{ backgroundImage: `url('${user.image}')` }}></div>}
      ...
      <div className="profile-info">
        ...
        <div>
          <b>Total Movies Watched:</b> {stats.totalMoviesWatched}
        </div>
        <div>
          <b>Last Movie Watched:</b>{" "}
          {stats.lastMovie ? (
            <Link to={`/movies/${stats.lastMovie.id}`}>{stats.lastMovie.title}</Link>
          ) : (
            "No movies watched"
          )}
        </div>
        <div>
          <b>Average Rating:</b> {stats.averageRating}
        </div>
        <div>
          <b>Highest Rated Movie:</b>{" "}
          {stats.highestRatedMovie ? (
            <Link to={`/movies/${stats.highestRatedMovie.id}`}>{stats.highestRatedMovie.title}</Link>
          ) : (
            "No ratings available"
          )}
        </div>
      ...
      {pendingRequests.length > 0 && (
        <div>
          ...
          {pendingRequests.map((request) => (
            <div key={request.id}>
              {users.find((u) => u.id === request.friendId)?.username}
              <button onClick={() => handleAcceptFriend(request)}>Accept</button>
              <button onClick={() => handleDenyFriend(request)}>Deny</button>
            </div>
          ))}
        </div>
      )}
